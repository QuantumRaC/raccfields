...
* add a `Makefile` target (`make test`) that runs pytest and a couple of smoke `curl` checks, or
* generate a one-click GitHub Actions CI snippet that runs the pytest above on push to `main`.
